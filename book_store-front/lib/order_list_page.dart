...
class OrderListPage extends StatefulWidget {
  ValueNotifier<BookRepository> booksNotifier;
  OrderListPage(this.booksNotifier, {super.key});
...
class _OrderListPageState extends State<OrderListPage> {
  ...
  void loadOrders() async {
    ...
    var response = await http.get(Uri.parse('$HOST/api/Orders/user/${currentUserNotifier.value.id}'),
          headers: {
            'Content-Type': 'application/json',
          }
        );

    List<dynamic> list = jsonDecode(utf8.decode(response.bodyBytes))['\$values'];
    ...
    for (int i = 0; i < ordersNotifier.value.length; i++) {
        int orderId = ordersNotifier.value[i].orderId;
        var orderItemResponse = await http.get(Uri.parse('$HOST/api/OrderItems/$orderId/items'),
          headers: {
            'Content-Type': 'application/json',
          }
        );
        List<dynamic> orderItemList = jsonDecode(utf8.decode(orderItemResponse.bodyBytes))['\$values'];
        for (int j = 0; j < orderItemList.length; j++){
          setState(() {
            ordersNotifier.value[i].orderEntries.add(OrderEntryDto.fromMap(orderItemList[j]));
             widget.booksNotifier.value.books.forEach((book) {
              if(book.id == ordersNotifier.value[i].orderEntries[j].bookId){
                  ordersNotifier.value[i].orderEntries[j].img =  book.preview;
              }
            });
          });
        }
        ...
        title: Row(
          children: [
            ...
            Column(
              ...
              children: [Text("№${order.orderId}"), const Text('success')],
            ),
            const Spacer(),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
                  ...
                  PopupMenuButton<String>(
                      ...
                      onSelected: (String item) {
                        ...
                        if (item == "orders_history") {
                          toOrderList(context, widget.booksNotifier);
                        }
                      },
