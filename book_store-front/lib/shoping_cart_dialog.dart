  ...
  void toOrder() async {
    if(credentials != null) {
      ...
      for (var i in orderEntities) {
        list.add(OrderEntryDto(i.count, i.book.id));
      }
      OrderDto order = OrderDto(currentUserNotifier.value.id,list);
      var post = await http.post(Uri.parse('${HOST}/api/Orders'),
